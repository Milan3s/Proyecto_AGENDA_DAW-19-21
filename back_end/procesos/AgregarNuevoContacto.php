...
if (isset($_POST['nuevo_contacto'])) {
    try {
        ...
        move_uploaded_file($_FILES['foto_contacto']['tmp_name'],$path.$nombre_foto_contacto); //insertar el nombre de la imagen en la base de datos y no la ruta.
         ...
         $query_insertar = $bd->prepare("INSERT INTO contactos (id_dispositivo,nombre,apellidos,cumple,telefono,ciudad,pais,idioma,email,foto)
         VALUES (?,?,?,?,?,?,?,?,?,?);");
        $resultado_contacto = $query_insertar->execute([$selectDispositivo,$nombre,$apellidos,$selectCumple,$telefono,$ciudad,$pais,$nombre_foto_idioma,$email,$nombre_foto_contacto]);
        
       if($resultado_contacto == true){
            $_SESSION['message_contacto'] = 'Contacto guardado correctamente.';
       }else{
           $_SESSION['message_contacto'] = 'El contacto no se ha podido guardar';
       }
       
        //Descomentar en caso de error

        //$query_insertar->debugDumpParams();
        /*var_dump($query_insertar);
        echo "<br>";
        var_dump($resultado);*/
    ...
    $_SESSION['message_contacto'] = 'Llene el formulario';
}
